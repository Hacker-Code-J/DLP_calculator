...
void Krtsb_test(BINT** pptrX, BINT** pptrY, BINT** pptrZ) {
    ...
    printf("Split X to X1: ");print_bint_hex_py(ptrX1);printf("\n");
    printf("Split X to X0: ");print_bint_hex_py(ptrX0);printf("\n");

    copyBINT(&ptrY1, pptrY); right_shift_word(&ptrY1, l);
    copyBINT(&ptrY0, pptrY); reduction(&ptrY0, l * WORD_BITLEN);
    printf("Split Y to Y1: ");print_bint_hex_py(ptrY1);printf("\n");
    printf("Split Y to Y0: ");print_bint_hex_py(ptrY0);printf("\n");

    refineBINT(*pptrX); refineBINT(*pptrY);

    Krtsb_test(&ptrX1, &ptrY1, &ptrT1);
    Krtsb_test(&ptrX0, &ptrY0, &ptrT0);
    printf("T1: ");print_bint_hex_py(ptrT1);printf("\n");
    printf("T0: ");print_bint_hex_py(ptrT0);printf("\n");

    copyBINT(&ptrShiftT1, &ptrT1);
    left_shift_word(&ptrShiftT1, 2*l);
    ADD(&ptrShiftT1, &ptrT0, &ptrR);
    printf("R: ");print_bint_hex_py(ptrR);printf("\n");

    SUB(&ptrX0, &ptrX1, &ptrS1);
    SUB(&ptrY1, &ptrY0, &ptrS0);
    printf("S1: ");print_bint_hex_py(ptrS1);printf("\n");
    printf("S0: ");print_bint_hex_py(ptrS0);printf("\n");

    bool sgn_S = ((ptrS0)->sign) ^ ((ptrS1)->sign);
    ptrS0->sign = false;
    ptrS1->sign = false;
    Krtsb_test(&ptrS1, &ptrS0, &ptrS);
    ptrS->sign = sgn_S;
    printf("S: ");print_bint_hex_py(ptrS);printf("\n");

    // copyBINT(&ptrTmpST1, &ptrS);
    ADD(&ptrS,&ptrT1,&ptrTmpST1);
    copyBINT(&ptrS,&ptrTmpST1);
    printf("S + t1: ");print_bint_hex_py(ptrS);printf("\n");

    copyBINT(&ptrTmpST0, &ptrS);
    ADD(&ptrTmpST0,&ptrT0,&ptrS);
    printf("S + t0: ");print_bint_hex_py(ptrS);printf("\n");

    left_shift_word(&ptrS, l);
    printf("S << lw: ");print_bint_hex_py(ptrS);printf("\n");

    ADD(&ptrR, &ptrS, pptrZ);
    printf("Result: ");print_bint_hex_py(*pptrZ);printf("\n");
